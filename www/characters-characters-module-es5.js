  ...
  function srcAppPagesCharactersCharactersPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var CharactersPage = /*#__PURE__*/function () {
      function CharactersPage(router, api) {
        ...
        this.router = router;
        this.api = api;
        this.offset = 0; //collection of character on the one page

        this.charId = null;
        this.maxPage = 50; //max num of charactrer that it going to display before activate infinite scroll function
      ...
      _createClass(CharactersPage, [{
        ...
        value: function ngOnInit() {
          this.loadCharacters(); //calling function 
        ...
        value: function loadCharacters(event) {
          ...
          this.api.getCharacters(this.offset).subscribe(function (res) {
            ...
            console.log("My Characters info:", res); //to display the info on the browser console, to verify if the api 
            //service is getting the data from the API service

            _this["char"] = _this["char"].concat(res);

            if (event) {
              event.target.complete(); //to finish the event
